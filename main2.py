# import pandas as pd
# from fastapi import FastAPI, File, UploadFile,Request
# from typing import List
# import io
# import json
# import aiohttp
# import asyncio

# app = FastAPI()


# async def get_coordinates(session, zip_code):
#     url = "https://nominatim.openstreetmap.org/search"
#     params = {
#         "postalcode": zip_code,
#         "country": "USA",
#         "format": "json"
#     }
#     async with session.get(url, params=params) as response:
#         data = await response.json()
#         if data and len(data) > 0:
#             return float(data[0]["lon"]), float(data[0]["lat"])
#         return None

# async def get_distance(session, start, end):
#     url = f"https://router.project-osrm.org/route/v1/driving/{start[0]},{start[1]};{end[0]},{end[1]}"
#     params = {"overview": "false"}
#     async with session.get(url, params=params) as response:
#         data = await response.json()
#         if data["code"] == "Ok":
#             return data["routes"][0]["distance"] / 1000  # Convert meters to kilometers
#         return None

# async def calculate_distance_matrix(input_data):
#     nodes = [0] + input_data["node_file"]["node"]
    
#     async with aiohttp.ClientSession() as session:
#         # Get coordinates for all nodes
#         tasks = [get_coordinates(session, str(zip_code)) for zip_code in nodes[1:]]
#         coordinates = await asyncio.gather(*tasks)
#         coordinates = [(0, 0)] + [coord for coord in coordinates if coord]  # Add dummy coordinates for node 0

#         # Calculate distances
#         distances = []
#         for i, start in enumerate(coordinates):
#             row = []
#             for j, end in enumerate(coordinates):
#                 if i == j:
#                     row.append(0)
#                 else:
#                     distance = await get_distance(session, start, end)
#                     row.append(distance if distance is not None else float('inf'))
#             distances.append(row)

#     # Create DataFrame
#     df = pd.DataFrame(distances, index=nodes, columns=nodes)
#     return df

# def process_input_and_calculate_matrix(input_data):
#     return asyncio.run(calculate_distance_matrix(input_data))

# # # Example usage
# # input_data = {
# #     "node_file": {
# #       "node": [10001, 10002, 10003, 10004, 10005, 10006, 10007, 10008, 10009, 10010],
# #       "demand": [0, 4, 6, 5, 4, 7, 3, 5, 4, 4]
# #     },
# #     "cap": 23
# # }
# # distance_matrix = process_input_and_calculate_matrix(input_data)
# # print(distance_matrix)






# # Function to calculate savings
# def calculate_savings(nodes, pw):
#     savings = dict()
#     for r in pw.index:
#         for c in pw.columns:
#             if int(c) != int(r):
#                 a = max(int(r), int(c))
#                 b = min(int(r), int(c))
#                 key = '(' + str(a) + ',' + str(b) + ')'
#                 savings[key] = nodes['d0'][int(r)] + nodes['d0'][int(c)] - pw[c][r]
    
#     sv = pd.DataFrame.from_dict(savings, orient='index')
#     sv.rename(columns={0: 'saving'}, inplace=True)
#     sv.sort_values(by=['saving'], ascending=False, inplace=True)
    
#     return sv

# # Helper functions
# def get_node(link):
#     link = link[1:-1]
#     nodes = link.split(',')
#     return [int(nodes[0]), int(nodes[1])]

# def interior(node, route):
#     try:
#         i = route.index(node)
#         return not (i == 0 or i == len(route) - 1)
#     except:
#         return False

# def merge(route0, route1, link):
#     if route0.index(link[0]) != len(route0) - 1:
#         route0.reverse()
#     if route1.index(link[1]) != 0:
#         route1.reverse()
#     return route0 + route1

# def sum_cap(route, nodes):
#     return sum(nodes.demand[node] for node in route)

# def which_route(link, routes):
#     node_sel, i_route = list(), [-1, -1]
#     count_in = 0
    
#     for route in routes:
#         for node in link:
#             try:
#                 route.index(node)
#                 i_route[count_in] = routes.index(route)
#                 node_sel.append(node)
#                 count_in += 1
#             except:
#                 pass
                
#     overlap = 1 if i_route[0] == i_route[1] else 0
#     return node_sel, count_in, i_route, overlap

# # Main routing function
# @app.post("/calculate-routes")
# async def calculate_routes(req:Request):
#     #nodes, pw = load_data(nodes_file, pw_file)
#     data = await req.json()
#     nodes=data['node_file']
#     pw=process_input_and_calculate_matrix(data)

#     nodes=pd.DataFrame(nodes)
#     # pw=pd.DataFrame(pw)

#     sv = calculate_savings(nodes, pw)
#     routes = list()
#     cap = 23
#     node_list = list(nodes.index)
#     node_list.remove(0)
#     remaining = True

#     for link in sv.index:
#         if remaining:
#             link = get_node(link)
#             node_sel, num_in, i_route, overlap = which_route(link, routes)
#             if num_in == 0:
#                 if sum_cap(link, nodes) <= cap:
#                     routes.append(link)
#                     node_list.remove(link[0])
#                     node_list.remove(link[1])
#             elif num_in == 1:
#                 n_sel = node_sel[0]
#                 i_rt = i_route[0]
#                 link_temp = link.copy()
#                 link_temp.remove(n_sel)
#                 node = link_temp[0]
#                 if not interior(n_sel, routes[i_rt]) and sum_cap(routes[i_rt] + [node], nodes) <= cap:
#                     routes[i_rt].append(node) if routes[i_rt].index(n_sel) != 0 else routes[i_rt].insert(0, node)
#                     node_list.remove(node)
#             else:
#                 if overlap == 0:
#                     cond1 = not interior(node_sel[0], routes[i_route[0]])
#                     cond2 = not interior(node_sel[1], routes[i_route[1]])
#                     cond3 = sum_cap(routes[i_route[0]] + routes[i_route[1]], nodes) <= cap
#                     if cond1 and cond2 and cond3:
#                         route_temp = merge(routes[i_route[0]], routes[i_route[1]], node_sel)
#                         routes.remove(routes[i_route[0]])
#                         routes.remove(routes[i_route[1]])
#                         routes.append(route_temp)
#                         node_list = [n for n in node_list if n not in link]
#         remaining = bool(len(node_list) > 0)

#     for node_o in node_list:
#         routes.append([node_o])

#     for route in routes:
#         route.insert(0, 0)
#         route.append(0)

#     return {"routes": routes}


import pandas as pd
from fastapi import FastAPI, Request
import aiohttp
import asyncio
import geopy.distance
from pyzipcode import ZipCodeDatabase

app = FastAPI()

async def get_coordinates(session, zip_code):
    zcdb = ZipCodeDatabase()
    zipcode = zcdb[zip_code]
    # print(zipcode.state)
    # print(zipcode.longitude, zipcode.latitude)
    return zipcode.latitude , zipcode.longitude
    # url = "https://nominatim.openstreetmap.org/search"
    # params = {
    #     "postalcode": zip_code,
    #     "country": "USA",
    #     "format": "json"
    # }
    # async with session.get(url, params=params) as response:
    #     data = await response.json()
    #     if data and len(data) > 0:
    #         return float(data[0]["lon"]), float(data[0]["lat"])
    #     return None

async def get_distance(session, start, end):
    # print(start, end)
    return geopy.distance.distance(start, end).km
    # url = f"https://router.project-osrm.org/route/v1/driving/{start[0]},{start[1]};{end[0]},{end[1]}"
    # params = {"overview": "false"}
    # async with session.get(url, params=params) as response:
    #     data = await response.json()
    #     if data["code"] == "Ok":
    #         return data["routes"][0]["distance"] / 1000  # Convert meters to kilometers
    #     return None

# async def calculate_distance_matrix(input_data):
#     nodes = input_data["node_file"]["node"]
#     print(nodes)
#     async with aiohttp.ClientSession() as session:
#         # Get coordinates for all nodes
#         tasks = [get_coordinates(session, str(zip_code)) for zip_code in nodes]
#         coordinates = await asyncio.gather(*tasks)
#         # coordinates = nodes[0]  
#         # print(coordinates)
#         # Calculate distances
#         distances = []
#         for i, start in enumerate(coordinates):
#             row = []
#             for j, end in enumerate(coordinates):
#                 if i == j:
#                     row.append(0)
#                 else:
#                     distance = await get_distance(session, start, end)
#                     row.append(distance if distance is not None else float('inf'))
#             distances.append(row)

#     # Create DataFrame
#     df = pd.DataFrame(distances, index=nodes, columns=nodes)
#     return df


async def calculate_distance_matrix(input_data):
    # Retrieve the original list of nodes
    original_nodes = input_data["node_file"]["node"]

    # Start creating a list of indices for these nodes
    nodes = list(range(len(original_nodes)))

    async with aiohttp.ClientSession() as session:
        # Get coordinates for all nodes
        tasks = [get_coordinates(session, str(zip_code)) for zip_code in original_nodes]
        coordinates = await asyncio.gather(*tasks)

        # Calculate distances between all nodes
        distances = []
        for i, start in enumerate(coordinates):
            row = []
            for j, end in enumerate(coordinates):
                if i == j:
                    row.append(0)
                else:
                    distance = await get_distance(session, start, end)
                    row.append(distance if distance is not None else float('inf'))
            distances.append(row)

    # Create a DataFrame with indices and columns starting from 0
    pw = pd.DataFrame(distances, index=nodes, columns=nodes)

    return pw

# Function to calculate savings
def calculate_savings(nodes, pw):
    savings = dict()
    for r in pw.index:
        for c in pw.columns:
            if int(c) != int(r):
                a = max(int(r), int(c))
                b = min(int(r), int(c))
                key = '(' + str(a) + ',' + str(b) + ')'
                savings[key] = nodes['d0'][int(r)] + nodes['d0'][int(c)] - pw[c][r]
                # savings[key] = pw[0][int(r)] + pw[0][int(c)] - pw[c][r]
    
    sv = pd.DataFrame.from_dict(savings, orient='index')
    sv.rename(columns={0: 'saving'}, inplace=True)
    sv.sort_values(by=['saving'], ascending=False, inplace=True)
    
    return sv

# Helper functions
def get_node(link):
    link = link[1:-1]
    nodes = link.split(',')
    return [int(nodes[0]), int(nodes[1])]

def interior(node, route):
    try:
        i = route.index(node)
        return not (i == 0 or i == len(route) - 1)
    except:
        return False

def merge(route0, route1, link):
    if route0.index(link[0]) != len(route0) - 1:
        route0.reverse()
    if route1.index(link[1]) != 0:
        route1.reverse()
    return route0 + route1

def sum_cap(route, nodes):
    return sum(nodes.demand[node] for node in route)

def which_route(link, routes):
    node_sel, i_route = list(), [-1, -1]
    count_in = 0
    
    for route in routes:
        for node in link:
            try:
                route.index(node)
                i_route[count_in] = routes.index(route)
                node_sel.append(node)
                count_in += 1
            except:
                pass
                
    overlap = 1 if i_route[0] == i_route[1] else 0
    return node_sel, count_in, i_route, overlap

# Main routing function
@app.post("/calculate-routes")
async def calculate_routes(req: Request):
    data = await req.json()
    nodes = data['node_file']
    
    # Calculate the distance matrix asynchronously
    pw = await calculate_distance_matrix(data)
    data["node_file"]["node"] = list(range(len(data["node_file"]["node"])))
    print(data)
    nodes['d0'] = pw.iloc[0].values
    # print(type (pw[10001]))
    nodes = pd.DataFrame(nodes)
    print(nodes)
    print(pw)
    sv = calculate_savings(nodes, pw)
    routes = list()
    cap = data["cap"]
    node_list = list(nodes.index)
    node_list.remove(0)
    remaining = True

    for link in sv.index:
        if remaining:
            link = get_node(link)
            node_sel, num_in, i_route, overlap = which_route(link, routes)
            if num_in == 0:
                if sum_cap(link, nodes) <= cap:
                    routes.append(link)
                    node_list.remove(link[0])
                    node_list.remove(link[1])
            elif num_in == 1:
                n_sel = node_sel[0]
                i_rt = i_route[0]
                link_temp = link.copy()
                link_temp.remove(n_sel)
                node = link_temp[0]
                if not interior(n_sel, routes[i_rt]) and sum_cap(routes[i_rt] + [node], nodes) <= cap:
                    routes[i_rt].append(node) if routes[i_rt].index(n_sel) != 0 else routes[i_rt].insert(0, node)
                    node_list.remove(node)
            else:
                if overlap == 0:
                    cond1 = not interior(node_sel[0], routes[i_route[0]])
                    cond2 = not interior(node_sel[1], routes[i_route[1]])
                    cond3 = sum_cap(routes[i_route[0]] + routes[i_route[1]], nodes) <= cap
                    if cond1 and cond2 and cond3:
                        route_temp = merge(routes[i_route[0]], routes[i_route[1]], node_sel)
                        routes.remove(routes[i_route[0]])
                        routes.remove(routes[i_route[1]])
                        routes.append(route_temp)
                        node_list = [n for n in node_list if n not in link]
        remaining = bool(len(node_list) > 0)

    for node_o in node_list:
        routes.append([node_o])

    for route in routes:
        route.insert(0, 0)
        route.append(0)

    return {"routes": routes}
